...
export default function EventCatering() {
  ...
  const menuPackages = [
    { id: 1, name: 'Classic Filipino Buffet', price: 450 },
    { id: 2, name: 'Highland Grill Special', price: 650 },
    { id: 3, name: 'Premium Wedding Feast', price: 850 }
  ];
  ...
  const selectedPkg = menuPackages.find(p => p.id == formData.package_id);
  const totalAmount = (selectedPkg?.price || 0) * (parseInt(formData.guests) || 0);
  ...
  const triggerConfirm = (e) => {
    ...
    if (selectedDate < today) {
      return showAlert("Invalid Date", "Event date cannot be in the past.");
    }

    if (isUnderage) {
      return showAlert("Age Restriction", "You must be 18 years or older to book an event.");
    }
    
    if (!formData.contact_number.startsWith("09") || formData.contact_number.length !== 11) {
      return showAlert("Invalid Number", "Contact number must start with 09 and contain exactly 11 digits.");
    }
  ...
  const executeBooking = async () => {
    ...
    try {
      ...
      showAlert("Inquiry Sent!", "We will review your event details and contact you shortly.", 'success');
    ...
    <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 pb-20 px-4 max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-[3rem] p-12 shadow-2xl border border-stone-100 max-w-4xl mx-auto">
        <form onSubmit={triggerConfirm} className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {/* EVENT DATE WITH CALENDAR CONSTRAINT */}
          <div className="space-y-2">
          ...
          <div className="md:col-span-2 space-y-2">
            ...
            <select required value={formData.package_id} onChange={(e) => setFormData({...formData, package_id: e.target.value})} className="w-full bg-stone-50 border-none rounded-2xl p-4 outline-none appearance-none cursor-pointer">
              ...
              {menuPackages.map(pkg => (<option key={pkg.id} value={pkg.id}>{pkg.name} (₱{pkg.price}/pax)</option>))}
            </select>
          </div>
          <div className="md:col-span-2 space-y-2">
            <label className="text-[10px] uppercase tracking-widest font-bold text-stone-400">Email Address (Optional)</label>
            <input type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="w-full bg-stone-50 border-stone-100 border rounded-2xl p-4 outline-none focus:ring-2 focus:ring-stone-200" placeholder="juan.delacruz@gmail.com" />
          </div>
          <div className="md:col-span-2 space-y-2">
          ...
          <div className="md:col-span-2 bg-[#1a2e1a] p-8 rounded-[2.5rem] text-white flex justify-between items-center">
            ...
            <div className="text-right">
              <p className="text-[10px] text-stone-400 font-bold uppercase">50% Downpayment Required</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] p-8 max-w-md w-full shadow-2xl transform animate-in zoom-in-95 duration-300">
            <div className="text-center">
              ...
              <p className="text-stone-500 text-sm mb-8 leading-relaxed">Are you sure you want to book <span className="font-bold">"{formData.event_name}"</span>?</p>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] p-8 max-w-md w-full shadow-2xl border border-stone-100 transform animate-in zoom-in-95 duration-300">
            <div className="text-center">
              ...
              <button onClick={handleCloseNotification} className="w-full bg-[#1a2e1a] text-white py-4 rounded-2xl font-bold hover:bg-orange-600 transition-all shadow-lg">
